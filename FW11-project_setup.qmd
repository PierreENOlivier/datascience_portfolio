...
```
# Web Scraping Data files
To expand on my knowledge and show that this project could easily update with newer version of the data, I will use web scraping to fetch the files from the repository.
Note that this type of data rarely get updates so it would be faster to just download
the files manually.

## Web Scraping setup
```{r}
url <- "https://figshare.com/collections/Who_eats_whom_in_the_Barents_Sea_a_food_web_topology_from_plankton_to_whales/3306768"


url_all <- "https://figshare.com/ndownloader/articles/3559350/versions/1"
``` 
...
page |> 
    ...
    html_attr("#a11y-1-tab-tab-download")
``` 

a11y-1-tab-tab-download

## Download data from source
```{r}


file_name <- "SpeciesList_BS.txt"
folder_path <- file.path("data", "raw_data", "Barents_Sea")
dir_create(folder_path)

``` 

```{r}
download.file(url, file.path(folder_path, file_name), mode = "wb")
``` 
